 ...
 *
 */
+(void)uploadImageWithOperations:(NSDictionary *)operations withImageArray:(NSArray *)imageArray withtargetWidth:(CGSize )origenSize withUrlString:(NSString *)urlString withSuccessBlock:(requestSuccess)successBlock withFailurBlock:(requestFailure)failureBlock withUpLoadProgress:(uploadProgress)progress;
{
    //1.创建管理者对象
    ...
    [manager POST:urlString parameters:operations constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (UIImage * image in imageArray) {
            ...
            //image的分类方法
            UIImage *  resizedImage = [image TS_imageByScalingAndCroppingForSize:CGSizeMake(800, 800*image.size.height/image.size.width)];//IMGCompressed:image targetWidth:width];
